...
4邻域$N_4(p)$即该像素上下左右的四个点，8邻域$N_8(p)$，对角邻域$N_D(p)$。
...
由于在$D_0$尾部包含高频分量$D_1$，处理后图像的清晰度较理想低通滤波器有所改善，但**会出现振铃效应**，
$$
...
通常采取$H(u,v)$下降到最大值的0.707时的$D(u,v)$作为截止频率，**没有振铃效应**。
...
- 一阶导数算子。
- 利用**局部差分平均方法**寻找边缘。**两个模板一个检测水平边缘，一个检测竖直边缘**。
- 对噪声具有一定的抑制能力。
